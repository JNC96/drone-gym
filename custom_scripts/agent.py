...
class Agent(object):
    ...
    def act(
        self, states, int_bool=False, int_act=None, parallel=0, deterministic=False, independent=False, evaluation=False,
        query=None, **kwargs
    ):
        """
        ...
        if kwargs is not None:
            ...
            for item in extra:
                if item=='inre':
                    inre = True
                    interactive_action = extra['interactive_action']
        else:
            inre = False
        # ****** NING EDIT END ***
        '''
